    ...
    layui.use(['table', 'layer', 'form'], function () {
        ...
        var tableIns = table.render({
            ...
            cols: [[
                ...
                {title: '操作', fixed: 'right', alien: 'center', toolbar: '#lineBarDemo'}
            ]]
        });//table-end

        //layui监听每行工具菜单的操作
        table.on('tool(userTableFilter)', function (obj) { //注：tool 是工具条事件名，userTableFilter 是 table 原始容器的属性 lay-filter="对应的值"
            ...
            if (layEvent === 'del') { //删除
                layer.confirm('你真的要删除吗?', {icon: 0, title: '提示'}, function (index) {
                    ...
                    //向服务端发送删除指令
                    //Ajax
                    $.get("delete_user?uid=" + data.uid, function (res) {
                        if (res.code == 0) {
                            //删除成功
        ...
        function addForm() {
            //设置表单呈现的默认值
            form.val("addFilter", { //editFilter 即 class="layui-form" 所在元素属性 lay-filter="" 对应的值
                "username": '' // "name": "value"
                , "password": ''
                , "telephone": ''
                , "sex": ''
            ...
            form.on('submit(add*)', function (formdata) {
                ...
                var jsonData = {
                    ...
                    , telephone: formdata.field.telephone
                    , sex: formdata.field.sex
                    , identity: formdata.field.identity
                    , grade: formdata.field.grade
                };
                $.ajax({
                    url: "add_user",   // 请求路径
